Deno.serve(async (req) => {
  try {
    ...
    if (!apiKey) return new Response(JSON.stringify({ error: 'GEMINI_API_KEY غير مهيأ.' }), { status: 503, headers: { 'Content-Type': 'application/json' } });
    if (!fileData || !mimeType) return new Response(JSON.stringify({ error: 'الملف أو نوعه غير موجود.' }), { status: 400, headers: { 'Content-Type': 'application/json' } });
    ...
    if (!response.ok) return new Response(JSON.stringify({ error: result.error?.message || 'فشل تحليل المنيو.' }), { status: 502, headers: { 'Content-Type': 'application/json' } });
    ...
    return new Response(JSON.stringify({ success: true, items, fileName }), { headers: { 'Content-Type': 'application/json' } });
