...
import { menuBaseName } from "./src/utils/businessRules";
...
async function getSettings() {
  ...
  if (!data) {
    const defaults = {
      whatsappNumber: "", deliveryPricingType: "area",
      distanceBaseFee: 0, distanceFeePerKm: 0,
      deliveryCommissionType: "flat", deliveryCommissionValue: 0,
      aboutUsContent: "", logoImage: "", deliveryOptions: [], coupons: [], categories: [], rewardOrderThreshold: 10,
    };
...
async function calculateOrderTotal(
  items: Array<{ menuItem: { id: string }; quantity: number; selectedSize?: { id?: string; name?: string }; restaurantId?: string }>,
  restaurantId: string,
  deliveryFee: number
): Promise<{ subtotal: number; total: number; validatedItems: any[] } | null> {

  const restaurantIds = [...new Set(items.map((item: any) => item.restaurantId || restaurantId))];
  const restaurantMap = new Map<string, any>();
  for (const id of restaurantIds) {
    const restaurant = await restaurants.get(id);
  ...
  for (const cartItem of items) {
    const itemRestaurantId = cartItem.restaurantId || restaurantId;
    const restaurant = restaurantMap.get(itemRestaurantId);
...
async function sendTelegramOrder(order: any): Promise<void> {
  ...
  const lines = [
    ...
    `المطعم: ${order.restaurant?.name || order.restaurantId}`,
...
app.post('/api/uploads/image', authenticateToken, canManageMenu, async (req, res) => {
  const { data, mimeType, fileName } = req.body || {};
  if (!data || !String(mimeType || '').startsWith('image/')) return res.status(400).json({ error: 'ملف صورة غير صالح.' });
  if (!process.env.CLOUDINARY_CLOUD_NAME || !process.env.CLOUDINARY_API_KEY || !process.env.CLOUDINARY_API_SECRET) return res.status(503).json({ error: 'خدمة تخزين الصور غير مهيأة.' });
  if (String(data).length > 12 * 1024 * 1024) return res.status(413).json({ error: 'الصورة كبيرة جدًا.' });
  try {
    const result: any = await cloudinary.uploader.upload(`data:${mimeType};base64,${data}`, { folder: 'mutawafer-eats', resource_type: 'image', use_filename: true, unique_filename: true, overwrite: false, transformation: [{ width: 1200, height: 1200, crop: 'limit', quality: 'auto', fetch_format: 'auto' }] });
    res.json({ url: result.secure_url, publicId: result.public_id, fileName });
  } catch (error: any) { res.status(502).json({ error: error.message || 'تعذر رفع الصورة.' }); }
});

// ────────────────────────────────────────────────────────────
// 🔌 WebSocket live updates for captain locations and orders
const socketSubscriptions = new Map<WebSocket, string>();
let socketServer: WebSocketServer | null = null;
function broadcastToOrder(orderId: string | undefined, event: string, payload: any) {
  if (!orderId) return;
  const message = JSON.stringify({ event, ...payload });
  socketSubscriptions.forEach((subscribedOrder, socket) => {
    if (subscribedOrder === orderId && socket.readyState === WebSocket.OPEN) socket.send(message);
  });
}
...
app.post("/api/admins/login", async (req, res) => {
  ...
  const token = generateToken({
    ...
    role:  (found.id === 'admin_primary' || found.role === 'primary') ? 'primary' : 'editor',
    // المدير الرئيسي له الصلاحيات كلها حتى لو كانت بياناته القديمة ناقصة في قاعدة البيانات.
    canManageRestaurants: found.role === 'primary' || found.id === 'admin_primary' ? true : found.canManageRestaurants === true || found.canManageRestaurants === 1,
    canManageMenu:        found.role === 'primary' || found.id === 'admin_primary' ? true : found.canManageMenu === true || found.canManageMenu === 1,
    canUseAIScanner:      found.role === 'primary' || found.id === 'admin_primary' ? true : found.canUseAIScanner === true || found.canUseAIScanner === 1,
  });
...
app.post("/api/captain/location", authenticateToken, async (req, res) => {
  ...
  broadcastToOrder(orderId, 'captain_location', { location: captainLocations.get(req.user.id) });
  res.json({ success: true });
});
...
app.post("/api/captain/location/offline", authenticateToken, async (req, res) => {
  const previous = captainLocations.get(req.user.id);
  captainLocations.delete(req.user.id);
  broadcastToOrder(previous?.orderId, 'captain_offline', { location: null });
  res.json({ success: true });
});
...
app.post("/api/orders", authenticateToken, orderLimiter, async (req, res) => {
  ...
  if (pricingMode === "distance") {
    ...
    const regions = Array.isArray(appSettings.deliveryOptions) ? appSettings.deliveryOptions : [];
    const region = regions.find((r: any) => String(r.id) === String(body.deliveryRegionId));
    if (!region) return res.status(400).json({ error: "الأدمن لم يضف مناطق توصيل وأسعارها بعد." });
    deliveryFee = Number(region.fee) || 0;
  }
  const restaurantIds = Array.isArray(body.restaurantIds) && body.restaurantIds.length ? body.restaurantIds : [body.restaurantId];
  const additionalRestaurantFee = Math.max(0, restaurantIds.length - 1) * 5;
  deliveryFee += additionalRestaurantFee;
  ...
  sendTelegramOrder(newOrder).catch(err => console.error('Telegram order notification error:', err));
...
app.put("/api/orders/:id/status", authenticateToken, async (req, res) => {
  ...
  broadcastToOrder(id, 'order_status', { order });
  res.json(order);
});
...
app.post("/api/restaurants/:id/menu", authenticateToken, canManageMenu, async (req, res) => {
  ...
  for (const item of items) {
    ...
    const key = `${menuBaseName(item.name)}::${String(item.category || '').trim().toLowerCase()}`;
    const existingIndex = menu.findIndex((old: any) => `${menuBaseName(old.name)}::${String(old.category || '').trim().toLowerCase()}` === key);
    if (existingIndex === -1) {
      menu.push(item);
    } else {
        ...
        socketServer.on('connection', (socket) => {
          socket.on('message', (raw) => {
            try {
              ...
              if (message.type === 'subscribe_order' && typeof message.orderId === 'string') {
                socketSubscriptions.set(socket, message.orderId);
                socket.send(JSON.stringify({ event: 'subscribed', orderId: message.orderId }));
              }
            } catch {}
          });
          socket.on('close', () => socketSubscriptions.delete(socket));
        });
        server.once("error", reject);
        server.listen(current, "0.0.0.0", () => resolve());
      });
      return current;
    } catch (error: any) {
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
  <url>
    <loc>https://mutawafer-eats.com/</loc>
