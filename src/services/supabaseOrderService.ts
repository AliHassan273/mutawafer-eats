...
export async function createOrderInSupabase(input: { cart: CartItem[]; customerName: string; customerPhone: string; address: string; paymentMethod: string; paymentDetails?: string; deliveryFee: number; additionalRestaurantFee: number; doorstepFee: number; discount: number; total: number; eta?: number }) {
  ...
  await supabase.functions.invoke('send-telegram-order', { body: { order: { ...order, customerName: input.customerName, customerPhone: input.customerPhone, deliveryAddress: input.address, total: input.total, deliveryFee: Number(input.deliveryFee || 0) + Number(input.additionalRestaurantFee || 0) + Number(input.doorstepFee || 0), items: input.cart.map((item, index) => ({ ...item, lineNumber: index + 1 })) } } }).catch(() => {});
