...
export default function App() {
  ...
  const { activeView, setActiveView, navigateTo, goBack } = useNavigation();
  ...
  const loyaltyStatus = useLoyalty(currentUser, orders, settings);
  ...
  const handleFinalizeOrder = async (
    ...
    const totalAmount = Math.max(0, subtotal + calculatedDeliveryFee + additionalRestaurantFee + (vodafoneFee || 0) + doorstepFee - itemDiscount);
  ...
  const filteredRestaurants = restaurants.filter((rest) => {
    if (!isCategoryVisible(selectedCategory)) return false;
  ...
  if (query) {
    restaurants.forEach((rest) => {
      if (!isCategoryVisible(selectedCategory)) return;
      let matchesCategory = true;
      if (selectedCategory !== 'all') {
        if (selectedCategory === 'offers') {
          matchesCategory = !!rest.promo;
      ...
      <main className="flex-1 pb-16">
        {activeView === 'home' && (
          <div className="space-y-6 animate-in fade-in duration-200">
            ...
            <LoyaltyCard loyaltyStatus={loyaltyStatus} whatsappNumber={settings.whatsappNumber} />
        ...
        {activeView === 'reviews' && <ReviewsPage reviews={reviews} onBack={goBack} />}
