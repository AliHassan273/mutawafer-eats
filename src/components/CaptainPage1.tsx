...
export default function CaptainPage({
  ...
  // Filters 
  // 1. Available orders are orders in 'Preparing' or 'Received' status with no assigned courier yet.
  ...
  // 2. My active deliveries are orders currently 'OutForDelivery' assigned to this captain's phone.
  const myActiveDeliveries = orders.filter(
    (o) => o.status === 'OutForDelivery' && o.courierPhone === currentUser.phone
  );

  // 3. My completed deliveries are orders marked 'Delivered' assigned to this captain.
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-6 sm:py-10 space-y-8" dir={isAr ? 'rtl' : 'ltr'}>
      ...
      {/* Critical Mandatory Requirement Banner: Keeping location ON at all times */}
      <div className="bg-gradient-to-r from-red-500/95 to-orange-600/95 text-white rounded-[24px] p-5 sm:p-6 shadow-lg relative overflow-hidden">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 relative z-10">
          <div className="space-y-2 max-w-3xl">
            ...
            <h3 className="text-base sm:text-lg font-black leading-tight">
              {isAr ? '⏰ متطلب أساسي: ابقِ موقع الـ GPS والموقع الجغرافي مفعلاً طوال الوقت!' : '⏰ MANDATORY: Keep Browser GPS / Location Active at All Times!'}
            </h3>
            <p className="text-xs sm:text-sm text-orange-50 leading-relaxed font-bold">
              {isAr 
                ? 'يجب إلزامياً إبقاء تطبيق مسافر قيد التشغيل والموقع الجغرافي نشطاً طوال فترة العمل لكي يتمكن السيرفر من تتبع إحداثياتك وإسناد الطلبات القريبة إليك تلقائياً وبأقصى سرعة.'
                : 'You are strictly required to keep Mutafer Eats running and your browser GPS/Location enabled at all times to securely receive order feeds and stream coordinates.'}
            </p>
            {gpsCoords && (
      ...
      {/* Strict GPS & Background Location Operational Enforcement Overlay */}
      {!isGpsAgreed && (
