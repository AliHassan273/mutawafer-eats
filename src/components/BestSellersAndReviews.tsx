  ...
  return (
    <div className="space-y-12 my-10 max-w-7xl mx-auto px-4 md:px-8" dir={isAr ? 'rtl' : 'ltr'}>
      ...
      <section className="space-y-6 pt-4 border-t border-slate-100">
        ...
        {reviews.length > 3 && onViewAllReviews && (
          <button onClick={onViewAllReviews} className="bg-slate-900 text-white rounded-xl px-4 py-2 text-xs font-black">عرض جميع الآراء</button>
        )}
        ...
        {reviews.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[...reviews].sort((a, b) => Date.parse(b.createdAt || "") - Date.parse(a.createdAt || "")).slice(0, 3).map((rev) => {
