...
export default function AdminCategories({ categories, setCategories, newId, setNewId, newNameAr, setNewNameAr, newIcon, setNewIcon, onSuccess, onRefresh, onDeleteCategory }: any) {
  return <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm space-y-4">
    ...
    <div className="space-y-2">{(categories as Category[]).map((cat, index) => <div key={cat.id} className="grid grid-cols-[auto_1fr_1fr_auto_auto] gap-2 items-center bg-slate-50 rounded-xl p-2">
      ...
      {cat.id !== 'all' && <button type="button" onClick={async () => { if (onDeleteCategory) await onDeleteCategory(cat.id); else setCategories((list: Category[]) => list.filter((_,i) => i !== index)); }} className="px-2 py-1 rounded-lg bg-red-50 text-red-600 text-[10px] font-black">حذف</button>}
    </div>)}</div>
    ...
    <button type="button" onClick={async () => { try { await saveSettingsToSupabase({ categories }); onSuccess('تم حفظ التصنيفات بنجاح ✅'); if (onRefresh) setTimeout(onRefresh, 500); } catch (err) { console.error('Save categories failed:', err); } }} className="bg-[#f94c10] text-white rounded-xl px-4 py-2 text-xs font-black">حفظ التصنيفات</button>
  </div>;
}
